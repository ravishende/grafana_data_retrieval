...
class Phase_4():
    ...
    def __init__(self, output_several_dfs: bool = False, files: dict[str, str] = PHASE_4_FILES, helitack_status: bool = False) -> None:
        ...
        wrong_type_msg = f"helitack_status must be a boolean, but was {type(helitack_status)}"
        assert isinstance(helitack_status, bool), wrong_type_msg
    ...
    def _update_queried_cols(self, df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _drop_zero_cpu_mem(self, df: pd.DataFrame, reset_index: bool = True) -> pd.DataFrame:
    ...
    def _insert_ratio_columns(self, df: pd.DataFrame, drop_numerators: bool = True, reset_index: bool = True) -> pd.DataFrame:

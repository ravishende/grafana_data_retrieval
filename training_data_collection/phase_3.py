...
class Phase_3():
    ...
    def _get_first_unqueried_row_idx(self, series):
        ...
        is_unqueried_series = series.apply(
            lambda row: not isinstance(row, (str, list)))
    ...
    def query_metrics(self, df, batch_size):
        ...
        try:
            ...
            progress_df = pd.read_csv(
                self.files['query_progress'], index_col=0)
        ...
        self._print_if_verbose(
            f"{num_queried_rows} rows queried, {num_rows_to_query} left.")
        self._print_if_verbose(f"Batch size: {batch_size} rows")
        batch_number = 1
        ...
        while not isinstance(df[self.col_names_total[-1]].iloc[len(df) - 1], (list, str)):
            ...
            df = query_and_insert_columns(
                df, self.static_metrics, self.col_names_static, self.duration_col_total, batch_size)
            ...
            if self.num_duration_cols > 0:
                for i in range(1, self.num_duration_cols+1):
                    ...
                    df = query_and_insert_columns(
                        df, self.non_static_metrics, col_names_t_i, self.duration_col_names[i-1], batch_size)

...
read_file = "csv_files/" #p2_ensemble_included.csv
write_file = "csv_files/" #calculated_training_data.csv
...
def get_ids(res_list):
    ...
    for item in res_list:
        ids.append(get_first_30(item['metric']['pod']))
    return ids

# get the csv file as a pandas dataframe
json_training_data = pd.read_csv(read_file, index_col=0)

# select columns to update (sum over)
ensemble_col = "ensemble"
cpu_tot_col = "cpu_usage"
mem_tot_col = "mem_usage"
cpu_t1_col = "cpu_t1"
mem_t1_col = "mem_t1"
cpu_t2_col = "cpu_t2"
mem_t2_col = "mem_t2"
...
print(calculated_training_data[cpu_tot_col].head(1000))
print("\n"*5)
print(calculated_training_data["worker_ids"].head(1000))
